...
export const openChannel: (id: string) => Promise<any> = async (channelPublicId: string) => {
    const Kakao = await getKakaoSDK();
    return Kakao.Channel.addChannel({channelPublicId});
}

export const openChannelChat: (id: string) => Promise<any> = async (channelPublicId: string) => {
    const Kakao = await getKakaoSDK();
    return Kakao.Channel.chat({channelPublicId});
}
